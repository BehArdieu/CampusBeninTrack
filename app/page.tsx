            ...
            {PARCOURS_PHASES.map((phase, i) => (
              <li key={phase.id}>
                <Link
                  ...
                  className="group flex h-full flex-col rounded-3xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-[0_12px_40px_-20px_rgb(42_79_68/0.55)]"
                >
                  <div className="flex items-start justify-between gap-2">
                    <span className="font-display text-2xl leading-none text-[var(--accent)]">{phase.icon}</span>
                    <span className="rounded-full bg-[var(--accent-soft)] px-2.5 py-0.5 text-xs font-semibold uppercase tracking-wide text-[var(--accent)]">
                      étape {(i + 1).toString().padStart(2, "0")}
                    </span>
                  </div>
                  <h2 className="mt-5 font-display text-xl font-semibold text-[var(--ink)] transition group-hover:text-[var(--forest)]">
                    {phase.shortTitle}
                  </h2>
                  <p className="mt-3 flex-1 text-sm leading-relaxed text-[var(--muted)]">{phase.summary}</p>
                  <span className="mt-6 text-sm font-semibold text-[var(--accent)] underline-offset-4 group-hover:underline">
                    Voir les consignes
                  </span>
                </Link>
              </li>
